...
def main():
    ...
    best_tree = carregar_modelo('melhor_modelo.pkl')
    ...
    if "diagnosticos_sugeridos" not in st.session_state:
        st.session_state.diagnosticos_sugeridos = []
    ...
    if st.session_state.diagnosticos_sugeridos:
        ...
        for diagnostico, sintoma in st.session_state.diagnosticos_sugeridos:
            ...
            checkbox_key = f"checkbox_{diagnostico}_{sintoma_selecionado}"
            if st.checkbox(f"Diagnóstico: {diagnostico}. Característica Definidora: {sintoma_selecionado}", key=checkbox_key):
                diagnosticos_selecionados.append(diagnostico)

        if diagnosticos_selecionados:
            st.session_state.diagnosticos_selecionados = diagnosticos_selecionados
        ...
        for diagnostico_selecionado in st.session_state.diagnosticos_selecionados:
            ...
            if diagnostico_selecionado in cuidados_df.columns:
                ...
                if cuidados_relacionados:
                    st.subheader("Cuidados Relacionados:")
                    cuidados_list = cuidados_relacionados.split('\t')
                    cuidados_list = random.sample(cuidados_list, min(5, len(cuidados_list)))
                    for cuidado in cuidados_list:
                        st.checkbox(cuidado, key=f"{diagnostico_selecionado}_{cuidado}")
